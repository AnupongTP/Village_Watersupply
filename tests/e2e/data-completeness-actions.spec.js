...
test('issue sections cap the first page at 20 rows and only sections above 20 expose show-more', async ({ page }) => {
  await installMockData(page, buildFixture(21, { firstCoordinateAt: 22 }));
  await openDashboard(page);
  await openIssues(page);

  const missing = page.locator('[data-issue-section="coordMissing"]');
  await expect(missing).toHaveAttribute('data-visible-count', '20');
  await expect(missing.locator('[data-issue-list] .issue-modal-row')).toHaveCount(20);
  await expect(missing.locator('[data-issue-status]')).toHaveText('แสดง 20 จาก 21 รายการ');
  await expect(missing.locator('[data-issue-more]')).toContainText('แสดงเพิ่มเติม');

  const village = page.locator('[data-issue-section="villagesWithoutSystem"]');
  await expect(village.locator('[data-issue-list] .issue-modal-row')).toHaveCount(1);
  await expect(village.locator('.issue-modal-footer')).toHaveCount(0);

  await missing.locator('[data-issue-more]').click();
  await expect(missing).toHaveAttribute('data-visible-count', '21');
  await expect(missing.locator('[data-issue-list] .issue-modal-row')).toHaveCount(21);
  await expect(missing.locator('[data-issue-status]')).toHaveText('แสดง 21 จาก 21 รายการ');
  await expect(missing.locator('[data-issue-more]')).toHaveCount(0);
});


test('exactly 20 rows does not render show-more', async ({ page }) => {
  await installMockData(page, buildFixture(20, { firstCoordinateAt: 21 }));
  await openDashboard(page);
  await openIssues(page);
...
test('show-more advances by 20 rows per click until the section is complete', async ({ page }) => {
  await installMockData(page, buildFixture(41, { firstCoordinateAt: 42 }));
  await openDashboard(page);
  await openIssues(page);

  const fortyOne = page.locator('[data-issue-section="coordMissing"]');
...
test('missing-coordinate system issues expose shared detail without inventing a map action', async ({ page }) => {
  ...
  await expect(missingSection.locator('[data-issue-action="map"]')).toHaveCount(0);
...
test('detail drawer returns to the same expanded completeness context', async ({ page }) => {
  ...
  const missing = page.locator('[data-issue-section="coordMissing"]');
  await missing.locator('[data-issue-more]').click();
  await expect(missing).toHaveAttribute('data-visible-count', '40');

  await page.locator('.swal-data-issues').evaluate(element => { element.scrollTop = 140; });
  const detail = missing.locator('[data-issue-action="detail"]').nth(25);
  await detail.click();
  await expect(page.locator('#systemDrawer')).toHaveClass(/open/);
  await page.locator('#btnCloseDrawer').click();

  await expect(page.locator('.swal2-title')).toHaveText('รายละเอียดความครบถ้วนของข้อมูล');
  const restored = page.locator('[data-issue-section="coordMissing"]');
...
test('high capacity values are not classified as data-completeness issues', async ({ page }) => {
  await installMockData(page, buildFixture(21, { firstCoordinateAt: 1 }));
  await openDashboard(page);

  await expect(page.locator('.completeness-issues strong')).toHaveText('1');
  await openIssues(page);
  await expect(page.locator('.swal2-popup')).not.toContainText('กำลังผลิตสูงผิดปกติ');
  await expect(page.locator('[data-issue-section="capacityOutlier"]')).toHaveCount(0);
});
...
test('data completeness modal keeps title and close actions outside the scrolling issue list', async ({ page }) => {
  ...
  const title = page.locator('.data-completeness-visible-header');
  ...
  const before = await Promise.all([popup.boundingBox(), title.boundingBox(), scroller.boundingBox(), actions.boundingBox(), topClose.boundingBox()]);
  expect(before.every(Boolean)).toBe(true);
  const [popupBox, titleBox, scrollerBox, actionsBox, closeBox] = before;
  expect(titleBox.y).toBeGreaterThanOrEqual(popupBox.y - 1);
  expect(titleBox.y + titleBox.height).toBeLessThanOrEqual(scrollerBox.y + 2);
  expect(scrollerBox.y + scrollerBox.height).toBeLessThanOrEqual(actionsBox.y + 2);
  expect(closeBox.x).toBeGreaterThanOrEqual(popupBox.x);
  expect(closeBox.x + closeBox.width).toBeLessThanOrEqual(popupBox.x + popupBox.width + 1);
  expect(closeBox.y).toBeGreaterThanOrEqual(popupBox.y);
  expect(closeBox.y + closeBox.height).toBeLessThanOrEqual(titleBox.y + titleBox.height + 1);

  await scroller.evaluate(element => { element.scrollTop = element.scrollHeight; });
  const after = await Promise.all([title.boundingBox(), actions.boundingBox()]);
  expect(after[0].y).toBeCloseTo(titleBox.y, 0);
  expect(after[1].y).toBeCloseTo(actionsBox.y, 0);
});
...
test('desktop completeness rows use compact three-column information density', async ({ page }) => {
  ...
  const section = page.locator('[data-issue-section="coordMissing"]');
...
test('360px modal preserves detail-action accessibility without clipping', async ({ page }) => {
  await page.setViewportSize({ width: 360, height: 800 });
  await installMockData(page, buildFixture(21));
  await openDashboard(page);
  await openIssues(page);

  const detail = page.locator('[data-issue-section="coordMissing"] [data-issue-action="detail"]').first();
  await expect(detail).toBeVisible();
  await expect(detail).toHaveAttribute('aria-label', /ดูรายละเอียด/);
  await expect(page.locator('[data-issue-section="coordMissing"] [data-issue-action="map"]')).toHaveCount(0);
